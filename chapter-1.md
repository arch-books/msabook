...
![alt text](./dreamride-core-backend.svg)

Виктор, как и любой опытный архитектор, организовал очень хорошо структурированный монолит, который состоит из десятков независимых модулей. На диаграмме выше в глаза бросаются сразу несколько компонентов: это наборы модулей (`module X/Y`) и хост приложения (`Host app`).

Давайте обсудим его немного подробнее и начнем с описания компонентов, которые в нём используются
...
- Общие модули и библиотеки (`shared lib 1`), которые представляют из себя транзитивные зависимости и могут быть использованы в любых других модулях
- Набор фиче-модулей (`module x`, `module y`), за разработку которых отвечают различные команды. Эти модули живут в отдельных репозиториях и подключаются в хост приложения посредством обычных библиотек. У каждого модуля свой репозиторий, релизный цикл и свой набор CI/CD пайплайнов.

На этом этапе у вас может появится множество вопросов. Как именно происходит загрузка модулей? Где хранятся контроллеры? Как модули взаимодействуют с базой данных?
